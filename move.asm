...
_do_run:
;	LINK	A5,#-$0000
	ST	_running(A4)	;_running
	CLR.B	_after(A4)	;_after
	MOVE.B	$0005(A7),_runch(A4)	;_runch
;	UNLK	A5
	RTS
...
_do_move:
	...
	CLR.B	_firstmove(A4)	;_firstmove
	TST.B	_bailout(A4)	;_bailout
	BEQ.B	L00091

	CLR.B	_bailout(A4)	;_bailout
...
L00091:
	TST.W	_no_move(A4)	;_no_move
	BEQ.B	L00092

	SUBQ.W	#1,_no_move(A4)	;_no_move
...
L00093:
	...
	ADD.W	$0008(A5),D3
	MOVE.W	D3,_nh2(A4)	;_nh + 2
	MOVE.W	_player+10(A4),D3	;_player + 10
	ADD.W	$000A(A5),D3
	MOVE.W	D3,_nh(A4)	;_nh
L00094:
	MOVE.W	_nh(A4),-(A7)	;_nh
	MOVE.W	_nh2(A4),-(A7)	;_nh + 2
	JSR	_offmap
	ADDQ.W	#4,A7
	TST.W	D0
	BNE.W	L0009B
...
L00095:
	...
	CLR.B	_after(A4)	;_after
L00096:
	MOVE.W	_nh(A4),d0	;_nh
	MOVE.W	_nh2(A4),d1	;_nh + 2
	JSR	_INDEXquick
...
L00098:
	...
	BRA.W	L00090
L00099:
	TST.W	_fall_level(A4)	;_fall_level
	BEQ.B	L0009A

	SUBQ.W	#1,_fall_level(A4)	;_fall_level
...
;	EXT.L	D0
	BRA.W	L000AF
L0009C:
	CMPI.W	#$0001,_player+12(A4)	;_player + 12
	BLE.B	L0009E
...
L0009F:
	...
	SUBQ.W	#1,D3
	MOVE.W	_player+12(A4),D2	;_player + 12
	CMP.W	D3,D2
	BGE.B	L000A1
...
L000A2:
	...
	MOVE.B	#$6B,_runch(A4)	;'k', _runch
	MOVE.W	#$FFFF,$0008(A5)
	BRA.B	L000A4
L000A3:
	MOVE.B	#$6A,_runch(A4)	;'j',_runch
	MOVE.W	#$0001,$0008(A5)
L000A4:
	CLR.W	$000A(A5)
	BRA.W	L00093
L000A5:
	CMPI.W	#$0001,_player+10(A4)	;_player + 10
	BLE.B	L000A7

	MOVE.W	_player+10(A4),D0	;_player + 10
	SUBQ.W	#1,D0
	MOVE.W	_player+12(A4),d1	;_player + 12
	JSR	_INDEXquick
...
L000A8:
	MOVE.B	D3,D6
	CMPI.W	#$003A,_player+10(A4)	;':',_player + 10
	BGE.B	L000AA
...
L000AB:
	...
	MOVE.B	#$68,_runch(A4)	;'h',_runch
	MOVE.W	#$FFFF,$000A(A5)
	BRA.B	L000AD
L000AC:
	MOVE.B	#$6C,_runch(A4)	;'l',_runch
	MOVE.W	#$0001,$000A(A5)
L000AD:
	CLR.W	$0008(A5)
	BRA.W	L00093
...
L000B9:
	...
	MOVE.W	_nh(A4),d1	;_nh
	MOVE.W	_nh2(A4),d0	;_nh +2
	JSR	_moatquick
...
L000BA:
	CLR.L	-(A7)
	MOVE.L	_cur_weapon(A4),-(A7)	;_cur_weapon
	MOVEQ	#$00,D3
	MOVE.B	D4,D3
	MOVE.W	D3,-(A7)
	PEA	_nh(A4)	;_nh
...
L000BC:
	...
	MOVE.W	_oldpos(A4),d0
	MOVE.W	_oldpos+2(A4),d1
	JSR	_INDEXquick
...
_be_trapped:
	...
	MOVE.B	#$01,_was_trapped(A4)	;_was_trapped
	MOVEQ	#$00,D0
	MOVE.B	D4,D0
	BRA.W	L000E0
L000CE:
...
L000D1:
	...
	MOVE.W	_player+32(A4),-(A7)	;_player + 32 AC
...
L000D7:
	...
	ADDQ.B	#1,_was_trapped(A4)	;_was_trapped
	BRA.W	L000E2
L000D8:
