...
;	LINK	A5,#-$0000
	...
	JSR	_init_weapon	;-$7DFE(A4)
	ADDQ.W	#6,A7
	MOVE.W	#$000F,-(A7)	;create 25-40 arrows
	JSR	_rnd		;-$7DD6(A4)
	ADDQ.W	#2,A7
	ADD.W	#$0019,D0
	MOVE.W	D0,$001E(A2)
...
1$	CLR.B	(A6)+
	...
	MOVEQ	#$00,D4
L00082:
	MOVE.W	#26,-(A7)	;26 different stones
	JSR	_rnd		;-$7DD6(A4)
	ADDQ.W	#2,A7
	MOVE.W	D0,D5
	LEA	-$001A(A5),A6
...
L001A7:
	...
	BRA.B	L001A6
L001A8:
	CMPI.W	#S_WILDMAGIC,$0020(A2)	;for scroll of wild magic, wisdom does not apply
	BEQ.B	L001A9
...
L001C7:
	...
	MOVE.W	#100,-(A7)	;10% chance you can't control it
	JSR	_rnd
	ADDQ.W	#2,A7
	CMP.W	#10,D0
	BGE	L001A6
...
_wild_magic:
	...
	MOVE.W	#$0006,-(A7)	;6 possible actions can happen
	JSR	_rnd		;-$7DD6(A4)
	ADDQ.W	#2,A7
;	EXT.L	D0
	BRA.W	L00214

; thunderclaps, removes all monster from the level
...
L002B0:
	...
	BNE.B	L002AD
L002B1:
	CMPI.W	#22,-$60AA(A4)	;max items in inventory
	BLT.B	L002B2
...
L002B2:
	...
	BRA.W	L002A7
L002B3:
	ORI.W	#$0008,$0028(A2)	;set scare bit used
L002B4:
	ADDQ.W	#1,-$60AA(A4)
	TST.B	D6
	BEQ.B	L002B5
...
L00493:
	...
	JSR	_msg		;-$7F76(A4)
	ADDQ.W	#4,A7
	ORI.W	#C_ISFOUND,-$52B4(A4)	;_player + 22
	PEA	-$52C0(A4)	;_player + 10
	JSR	_leave_room	;-$7D06(A4)
	ADDQ.W	#4,A7
	PEA	-$52C0(A4)	;_player + 10
...
L00494:
	...
	BRA.B	L00496
L00495:
	ORI.W	#C_WISDOM,-$52B4(A4)	;WISDOM, _player + 22
...
;	LINK	A5,#-$0000
	...
	ORI.W	#C_CANSEE,-$52B4(A4)	;_player + 22
...
2$	ORI.W	#C_ISHUH,-$52B4(A4)	;C_ISHUH,_player + 22
;	UNLK	A5
	RTS
...
;	LINK	A5,#-$0000
	...
	ORI.W	#C_ISBLIND,-$52B4(A4)	;_player + 22
...
_lose_vision:
;	LINK	A5,#-$0000
	ANDI.W	#~C_ISFOUND,-$52B4(A4)	;clear C_ISFOUND,_player + 22
	PEA	-$52C0(A4)	;_player + 10
	JSR	_leave_room	;-$7D06(A4)
	ADDQ.W	#4,A7
	PEA	-$52C0(A4)	;_player + 10
...
;	MOVEA.L	-$5190(A4),A6	;_cur_ring_1
	MOVE.W	$0026(A6),D3
	SUB.W	D3,-$000A(A5)	;add extra armor from ring
L00997:
	TST.L	-$518C(A4)	;_cur_ring_2
	BEQ.B	L00998
...
;	MOVEA.L	-$518C(A4),A6	;_cur_ring_2
	MOVE.W	$0026(A6),D3	;add extra armor from ring
	SUB.W	D3,-$000A(A5)
L00998:
	MOVE.L	-$0004(A5),-(A7)
	JSR	_atoi(PC)
...
1$	CLR.W	-(A7)
	...
	ADDQ.W	#8,A7
2$
	ORI.W	#C_ISHUH,-$52B4(A4)	;set C_ISHUH,_player + 22
